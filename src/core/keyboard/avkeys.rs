...
use crate::{
    config::templating::{
        avvalue::{AvValue, UnexpectedType},
        r#macro::ParameterError,
        AvDeserialize, MacroParameter,
    },
...
impl Clone for AvKeys {
    fn clone(&self) -> Self {
        Self(self.0.iter().map(|k: &AvKey| (*k).clone()).collect())
    }
}
...
#[AvError(
    TraceableError,
    CONFIG_KEY_PARSE,
    "Config: Keyboard Shortcut Parsing Error"
)]
...
impl AvDeserialize for AvKeys {
    fn deserialize(
        ...
        match val {
            Value::String(s) => {
                Ok(AvValue::AvKeys(AvKeys::try_from(s).map_err(
                    |(t, v)| match t.as_str() {
                        "key" => Box::new(KeyParseError(loc, v)) as Box<dyn TraceableError>,
                        "macro_param" => {
                            Box::new(ParameterError(loc, v)) as Box<dyn TraceableError>
                        }
                        _ => unreachable!(),
                    },
                )?))
...
mod tests {
    ...
    fn deserialize() {
        ...
        assert_eq!(
            v,
            Ok(AvKeys(vec![
                AvKey::Key("Ctrl".to_string()),
                AvKey::Parameter(MacroParameter::DigitKey)
            ]))
        )
    }
}
