...
use compositor_macros::{description, location, AvError, AvValue};

use crate::core::{
    error::{AvError, Traceable, TraceableError},
    keyboard::{avkeys::AvKey, AvKeys},
};

use serde_json::Value;

use super::{
    r#macro::{AvKeysMismatch, Difference},
    AvMacro, MacroParameter,
};
...
impl AvValue {
    pub fn parse_same_type(
        &self,
        loc: Traceable,
        val: Value,
    ) -> Result<Self, Box<dyn TraceableError>> {
        match &self {
...
impl AvDeserialize for i64 {
    fn deserialize(loc: Traceable, val: Value) -> Result<AvValue, Box<dyn TraceableError>> {
        match val {
            Value::Number(ref v) => match v.as_i64() {
                Some(v) => Ok(AvValue::i64(v.clone())),
                None => Err(Box::new(UnexpectedType::from(
                    loc,
                    "Integer",
                    Into::<Value>::into(v.clone()),
                ))),
            },
...
impl AvDeserialize for f64 {
    fn deserialize(loc: Traceable, val: Value) -> Result<AvValue, Box<dyn TraceableError>> {
        match val {
            Value::Number(ref v) => match v.as_f64() {
