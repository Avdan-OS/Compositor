...
use compositor_macros::{description, location, AvError};

use crate::core::error::{AvError, Traceable, TraceableError};

use std::{collections::HashSet, convert::TryFrom, hash::Hash, iter::FromIterator};
...
#[AvError(
    TraceableError,
    CONFIG_MACRO_PARAMETER_ERROR,
    "Config: Macro Parameter Error"
)]
...
impl AvMacro {
    ...
    pub fn has_parameters(
        &self,
        v: Vec<MacroParameter>,
    ) -> Result<(), (Difference, Vec<MacroParameter>)> {
        match self.parameters.len() {
            l if l < v.len() => Err((
                Difference::FromNew,
                v.iter()
                    .filter(|k: &&MacroParameter| !self.parameters.contains(k))
            ...
            l if l > v.len() => Err((
                ...
                self.parameters
                    .iter()
                    .filter(|k: &&MacroParameter| !v.contains(k))
                    .map(|k: &MacroParameter| k.clone())
                    .collect(),
            )),
    ...
    pub fn parse(loc: Traceable, value: String) -> Result<AvMacro, Vec<Box<dyn TraceableError>>> {
        ...
        if value.len() == 0 {
            return Err(vec![Box::new(MacroEmpty(loc))]);
        }
        ...
        for (index, chr) in value.chars().enumerate() {
            ...
            match current_state {
                State::Identifier => match chr {
                    ...
                    _ => {
                        return Err(vec![Box::new(ParseError {
        ...
        match current_state {
            State::Identifier => ident = current_token,
        ...
        let errs: Vec<_> = parameters
            ...
            .map(|(i, r)| {
                ...
                Box::new(ParameterError(e.clone(), err)) as Box<dyn TraceableError>
            })
            .collect();
...
#[AvError(
    TraceableError,
    CONFIG_MACRO_SIGNATURE_MISMATCH,
    "Config: Macro Signature Mismatch"
)]
pub struct SignatureMismatchError(
    pub Traceable,
    pub String,
    pub (Difference, Vec<MacroParameter>),
);
...
#[AvError(
    TraceableError,
    CONFIG_AVKEYS_PARAMETER_MISMATCH,
    "Config: Macro Value Mismatch"
)]
pub struct AvKeysMismatch(
    pub Traceable,
    pub String,
    pub (Difference, Vec<MacroParameter>),
);
...
mod tests {
    ...
    #[test]
    fn parsing_test() {
        let m: Result<AvMacro, Vec<Box<dyn TraceableError>>> =
            AvMacro::parse(traceable!(), "helpMe(deez)".to_string());
