...
use smithay::{
    ...
    delegate_layer_shell, delegate_shm, delegate_viewporter,
    desktop::{
...
#[derive(Default)]
pub struct FullscreenSurface(RefCell<Option<AvWindow>>);

impl FullscreenSurface {
    pub fn set(&self, window: AvWindow) {
        *self.0.borrow_mut() = Some(window);
    }

    pub fn get(&self) -> Option<AvWindow> {
        self.0.borrow().clone()
    }

    pub fn clear(&self) -> Option<AvWindow> {
        self.0.borrow_mut().take()
    }
}

impl<BEnd: Backend> BufferHandler for Navda<BEnd> {
    fn buffer_destroyed(&mut self, _buffer: &WlBuffer) {}
}

impl<BEnd: Backend> CompositorHandler for Navda<BEnd> {
    ...
    fn commit(&mut self, surface: &WlSurface) {
        ...
        if !is_sync_subsurface(surface) {
            ...
            if let Some(AvWindow::Wayland(window)) = self.window_for_surface(&root) {
                window.on_commit();
            }
        }
...
impl<BEnd: Backend> WlrLayerShellHandler for Navda<BEnd> {
...
delegate_layer_shell!(@<BEnd : Backend + 'static> Navda<BEnd>);

impl<BEnd: Backend> Navda<BEnd> {
    pub fn window_for_surface(&self, surface: &WlSurface) -> Option<AvWindow> {
        self.space
            .elements()
...
fn ensure_initial_configure(
        ...
        .cloned()
    {
        ...
        if let AvWindow::Wayland(ref toplevel) = window {
        ...
        with_states(surface, |states| {
            ...
            if let ResizeState::WaitingForCommit(_) = data.resize_state {
                data.resize_state = ResizeState::Idle;
            }
        });
...
impl<BEnd: Backend + 'static> ShmHandler for Navda<BEnd> {
    fn shm_state(&self) -> &smithay::wayland::shm::ShmState {
        &self.shm_state
    }
}

delegate_shm!(@<BEnd : Backend + 'static> Navda<BEnd>);

delegate_viewporter!(@<BEnd : Backend + 'static> Navda<BEnd>);

///
/// Decides where to place a new window.
///
fn place_new_window(space: &mut Space<AvWindow>, window: &AvWindow, activate: bool) {
    // TODO(Sammy99jsp): Change from `anvil`'s default below for a more fancy impl:
...
pub fn fixup_positions(space: &mut Space<AvWindow>) {
    // fixup outputs
