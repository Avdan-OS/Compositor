//!
//! The `udev` backend allows us to directly
//! interface with the drm (Display Rendering Manager)
//! which allows to draw pixels to the screen.
//!
//! Also:
//! * we interact with graphics hardware, too.
//! * start user sessions here.
//! * other processes.
//!
//! This module is basically a copy-paste from anvil's
//! `udev` backend, so this will be heavily worked on later.
//!

// NOTE: We are also including the EGL features by default.
//       So, yup..

// TODO(Sammy99jsp) REVIEW THIS ENTIRE MODULE:
...
// EGL
impl DmabufHandler for Navda<UdevData> {
...
pub fn run_udev(log: Logger) {
    ...
    let dmabuf_state = {
        ...
        if renderer.bind_wl_display(&display.handle()).is_ok() {
            ...
            let global = state.create_global::<Navda<UdevData>, _>(
                &display.handle(),
                dmabuf_formats,
                log.clone(),
            );
...
pub type RenderSurface =
    GbmBufferedSurface<GbmDevice<DrmDeviceFd>, Option<OutputPresentationFeedback>>;
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<Navda<UdevData>>(global);
        }
    }
}
...
fn scan_connectors(
    ...
    device: &DrmDevice,
    gbm: &GbmDevice<DrmDeviceFd>,
    display: &mut Display<Navda<UdevData>>,
    ...
    for connector_info in connector_infos {
        ...
        for crtc in crtcs {
            ...
            let interface_short_name = match connector_info.interface() {
                connector::Interface::DVII => Cow::Borrowed("DVI-I"),
                connector::Interface::DVID => Cow::Borrowed("DVI-D"),
                connector::Interface::DVIA => Cow::Borrowed("DVI-A"),
                connector::Interface::SVideo => Cow::Borrowed("S-VIDEO"),
                connector::Interface::DisplayPort => Cow::Borrowed("DP"),
                connector::Interface::HDMIA => Cow::Borrowed("HDMI-A"),
                connector::Interface::HDMIB => Cow::Borrowed("HDMI-B"),
                connector::Interface::EmbeddedDisplayPort => Cow::Borrowed("eDP"),
            ...
            let global = output.create_global::<Navda<UdevData>>(&display.handle());
            let position = (
                space
                    .outputs()
    ...
    fn device_changed(&mut self, display: &mut Display<Self>, device: dev_t) {
        ...
        if let Some(ref mut backend_data) = self.backend_data.backends.get_mut(&node) {
            ...
            *backends = scan_connectors(
            ...
            shell::fixup_positions(&mut self.space);
    ...
    fn frame_finish(
        ...
        let device_backend = match self.backend_data.backends.get_mut(&dev_id) {
            ...
            None => {
                slog::error!(
                    self.log,
                    "Trying to finish frame on non-existent backend {}",
                    dev_id
                );
                return;
            }
        ...
        let surface = match surfaces.get(&crtc) {
            ...
            None => {
                slog::error!(
                    self.log,
                    "Trying to finish frame on non-existent crtc {:?}",
                    crtc
                );
                return;
            }
